...
This images above were developed in a [jupyter notebook](https://jupyter.org/) that can be found here: [notebook.ipynb](notebook.ipynb). The notebook has been dumped to a HTML here [notebook.html](notebook.html) (with the command `jupyter nbconvert --to html notebook.ipynb`).
